...
export default function SignupPage() {
  ...
  return (
    // ✅ Added overflow-y-auto to handle landscape mode on mobile
    <div className="min-h-screen bg-[#252836] flex items-center justify-center p-4 sm:p-6 font-sans overflow-y-auto">
      
      {/* ✅ Responsive max-width and dynamic padding (p-6 on mobile, p-10 on sm+) */}
      <div className="bg-[#1F1D2B] w-full max-w-[420px] p-6 sm:p-10 rounded-2xl border border-gray-800 shadow-2xl my-auto">
        
        <div className="text-center mb-8 sm:mb-10">
          <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2">Join Jaegar Resto</h1>
          <p className="text-gray-400 text-sm sm:text-base">Create an account to start ordering</p>
        </div>
        ...
        <p className="text-center text-gray-500 mt-6 sm:mt-8 text-xs sm:text-sm">
          Already have an account? <Link href="/login" className="text-[#EA7C69] cursor-pointer hover:underline">Log In</Link>
        </p>
      </div>
    </div>
  );
